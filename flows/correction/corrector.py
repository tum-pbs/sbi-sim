...
from simulations import SimulationTargets, LotkaVolterraSimulator
from simulations.sbi_simulators import SBISimulator
...
class Corrector(nn.Module):
    ...
    clip_value_grad: float = 2.0
    clip_value_loss: float = 12.0
    start_time: float = 1.0

    def setup(self):

        self.simulation_model = instantiate_from_config(self.simulation["setup"])
        self.targets: SimulationTargets = (
            instantiate_from_config(self.simulation["target"]))
        self.target_mask = self.targets.get_target_mask()

        self.aggregation_model = instantiate_from_config(self.aggregation)

    def simulate(self, x, observation_params):

        rng = jr.PRNGKey(0)

        prediction_full = self.targets.assemble(observation_params, x)

        return self.simulation_model.forward(prediction_full, rng, False, True)

    def negative_log_likelihood(self, x, observation_image, observation_params):

        observation_pred = self.simulate(x, observation_params)

        loss = - self.simulation_model.compute_log_likelihood_per_pixel(observation_pred, observation_image)

        return loss

    def __call__(self, t, x, observation, train=True):

        return self.forward(t, x, observation, train=train)[0]

    def forward_flow(self, t, x, observation, train=False):

        # we need this because self.model was trained with stacked flow which has additional time dimensions
        t_in_model = jnp.concatenate([jnp.ones_like(t), t], axis=1)
        return self.model(t_in_model, x, observation, train=train)

    def forward(self, t, x, observation, train=True):

        # we need this because self.model was trained with stacked flow which has additional time dimensions
        t_in_model = jnp.concatenate([jnp.ones_like(t), t], axis=1)

        # predict flow
        flow_pred = self.model(t_in_model, x, observation, train=train)

        if self.freeze:
            flow_pred = stop_gradient(flow_pred)

        x_1 = x + jnp.einsum('ab,a->ab', flow_pred, 1 - t[:, 0])

        grad_fn = vmap(value_and_grad(self.negative_log_likelihood), in_axes=(0, 0, 0))
        loss, grad = grad_fn(x_1, *observation)
        ...
        if self.layer_norm:
            G = jnp.einsum('ab, a -> ab', G, 1 / jnp.linalg.norm(G, axis=1, keepdims=False))
        else:
            G = jnp.clip(G, -self.clip_value_grad, self.clip_value_grad)

        loss = jnp.nan_to_num(loss)
        loss = jnp.clip(loss, -self.clip_value_loss, self.clip_value_loss)

        t = jnp.concatenate([t, loss], axis=1)

        drift = flow_pred + self.aggregation_model(t, flow_pred, G)

        drift = (jnp.einsum('ab, a -> ab', drift, t[:,0] > self.start_time) +
                 jnp.einsum('ab, a -> ab', flow_pred, t[:,0] <= self.start_time))

        return drift, loss, grad
