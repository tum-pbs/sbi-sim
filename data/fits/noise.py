import numpy as np
# Same as noise in herculens but using only numpy so that it can be used with Torch DataLoader and multiprocessing

def add_background_fn(image, sigma_bkd):
    """
...
def add_poisson_fn(image, exp_time):
    """
...
class NumpyNoise(object):
    ...
    def realisation(self, model, add_gaussian=True, add_poisson=True):
        ...
        if add_poisson and self._exp_map is not None:
            noise_real += add_poisson_fn(model, self._exp_map)
        if add_gaussian:
            noise_real += add_background_fn(model, self._background_rms)
        return noise_real
    ...
    def covariance_matrix(data, background_rms, exposure_map):
        ...
        :param data: data array, e.g. in units of photons/second
        :param background_rms: background noise rms, e.g. in units (photons/second)^2
        ...
        if exposure_map is not None:
            ...
            sigma2 = sigma2 * np.ones_like(data)
        return sigma2